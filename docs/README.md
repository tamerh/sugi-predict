...
Predictions are **predicted, not catalogued**: each carries a `confidence` (best Tanimoto to a
backing ChEMBL ligand) and a `support` count (n of 20 neighbours agreeing); below ~0.3 is
novel/whitespace chemistry. Chemistry is the only relationship validated as predictive — the
supporting substrate (patent text, trials, proteins) is retrieval context, not corroboration.
...
- **[how-it-works.md](how-it-works.md)** — the prediction method (exact-Tanimoto k-NN over the
  ChEMBL reference), provenance join, the substrate, and what the honesty signals mean.
