#!/bin/bash
# Pubmed UPDATE — embed one BATCH of baseline files (PMID-delta: skip already-embedded).
# Throttled: OMP/MKL threads=2 per task; run with Enju parallel=8 => ~16 of 32 cores.
set -euo pipefail
source /data/miniconda3/etc/profile.d/conda.sh
conda activate bioyoda
cd "${BIOYODA_ROOT:-/data/bioyoda}"
export OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2

BATCH="$ENJU_PARAM_batch"                 # e.g. "00".."29"
BATCH_SIZE="${PUBMED_BATCH_SIZE:-45}"
RAW=out/raw_data/pubmed/baseline
OUT=out/data/processed/pubmed/baseline
mkdir -p "$OUT"

mapfile -t files < <(ls "$RAW"/*.xml.gz 2>/dev/null | sort)
start=$(( 10#$BATCH * BATCH_SIZE ))
slice=( "${files[@]:start:BATCH_SIZE}" )
echo "batch $BATCH: files ${start}..$((start+${#slice[@]}-1)) (${#slice[@]} files)" >&2

for f in "${slice[@]}"; do
    python modules/pubmed/scripts/index.py "$f" "$OUT" \
        --deleted-pmids out/state/pubmed/none.gz \
        --existing-pmids out/state/pubmed/existing_pmids.txt.gz \
        --model-name "pritamdeka/S-BioBERT-snli-multinli-stsb" --vector-dim 768
done
echo "batch $BATCH done (${#slice[@]} files)" >&2
