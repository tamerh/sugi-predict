...
mapfile -t files < <(ls work/raw_data/pubmed/baseline/*.xml.gz \
                        work/raw_data/pubmed/updatefiles/*.xml.gz 2>/dev/null | sort)
start=$(( 10#$BATCH * BATCH_SIZE ))
slice=( "${files[@]:start:BATCH_SIZE}" )
echo "batch $BATCH: files ${start}..$((start+${#slice[@]}-1)) (${#slice[@]} files)" >&2

DELETED=work/raw_data/pubmed/deleted.pmids.sorted.gz
[[ -f "$DELETED" ]] || DELETED=work/state/pubmed/none.gz
...
for f in "${slice[@]}"; do
    $PIN python modules/pubmed/scripts/index.py "$f" "$OUT" \
        ...
        --existing-pmids work/state/pubmed/existing_pmids.txt.gz \
