...
def main():
    ...
    # RESUME: if dst already holds the full data (a prior run copied but crashed before green), skip the copy
    if qc.collection_exists(a.dst) and qc.count(a.dst).count >= total * 0.999:
        print(f"  RESUME: {a.dst} already has {qc.count(a.dst).count:,} points -> skipping copy, (re)building HNSW", flush=True)
    else:
    ...
    while True:
        try:
            ...
            if str(i.status).endswith("green") and (i.indexed_vectors_count or 0) >= i.points_count * 0.99:
                break
        except Exception as e:                       # survive a transient connection blip (don't crash the build wait)
            print(f"  (transient on poll: {str(e)[:50]}; retrying)", flush=True)
        time.sleep(30)
