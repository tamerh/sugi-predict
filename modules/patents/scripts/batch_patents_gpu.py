"""
Batch Patent Processing for Google Colab GPU
Processes multiple parquet chunks with resume capability and state tracking.
...
Usage in Colab:
    ...
    # Run processing
    !python batch_patents_gpu.py \
        --input-dir /content/drive/MyDrive/bioyoda/raw_data/patents/chunked \
        --output-dir /content/drive/MyDrive/bioyoda/processed/patents/text \
        --state-file /content/drive/MyDrive/bioyoda/state/patents/processed_chunks.json \
        --uspto-data /content/drive/MyDrive/bioyoda/raw_data/patents/historical_uspto/uspto_historical.parquet
"""
import os
import sys
import glob
...
def get_pending_chunks(input_dir, output_dir, state):
    ...
    # Find all input parquet chunks
    pattern = os.path.join(input_dir, "patents_chunk_*.parquet")
...
def main():
    parser = argparse.ArgumentParser(description="Batch GPU processing for patents")
    parser.add_argument("--input-dir", required=True, help="Directory with parquet chunks")
    ...
    parser.add_argument("--uspto-data", default=None, help="USPTO parquet for enrichment")
    ...
    from process_patents_gpu import (
        process_chunk, load_model_gpu, detect_gpu, load_uspto_lookup
    )

    # Load USPTO data once if provided
    uspto_lookup = None
    if args.uspto_data and os.path.exists(args.uspto_data):
        log(f"Loading USPTO data for enrichment...")
        uspto_lookup = load_uspto_lookup(args.uspto_data)
    else:
        log("No USPTO data provided, processing titles only")
